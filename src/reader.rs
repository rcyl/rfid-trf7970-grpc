...
impl Reader {
    ...
    pub fn new(serial: Box<dyn RFIDSerialTraits>) 
        -> Reader {
...
mod test {
    ...
    mod init {
        ...
        fn serial_port_error() {
            ...
            serial.expect_send_recv().with(eq(ISO)).returning(|_| {
        ...
        fn agc_res_two_ok() {
            let mut serial = MockRFIDSerialTraits::new();
            serial.expect_send_recv().with(eq(ISO)).returning(|_| {
                Ok(String::from(ISO_RES))
            });
            serial.expect_send_recv().with(eq(RF_HALF_DATA)).returning(|_| {
                Ok(String::from(RF_HALF_DATA_RES))
            });
            serial.expect_send_recv().with(eq(AGC)).returning(|_| {
                Ok(String::from(AGC_RES_2))
            });
            serial.expect_send_recv().with(eq(AM)).returning(|_| {
                Ok(String::from(AM_RES))
            });
            serial.expect_send_recv().with(eq(EXT_ANT)).returning(|_| {
                Ok(String::from(EXT_ANT_RES))
            });
            let mut _reader = Reader::new(Box::new(serial));
        }
        ...
        fn am_res_two_ok() {
            ...
            serial.expect_send_recv().with(eq(AGC)).returning(|_| {
                Ok(String::from(AGC_RES))
            });
            serial.expect_send_recv().with(eq(AM)).returning(|_| {
                Ok(String::from(AM_RES_2))
            });
            serial.expect_send_recv().with(eq(EXT_ANT)).returning(|_| {
                Ok(String::from(EXT_ANT_RES))
            });
